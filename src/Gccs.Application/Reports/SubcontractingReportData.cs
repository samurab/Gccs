    ...
    IApplicationTransaction transaction)
{
    ...
    public Task<SubcontractingReportDataRowDto> CreateAsync(
        ...
        transaction.ExecuteAsync(async token =>
        {
            ...
            await ValidateAsync(normalized, null, token);
    ...
    public async Task<SubcontractingReportDataRowDto?> UpdateAsync(
        ...
        await transaction.ExecuteAsync(async token =>
        {
            ...
            var normalized = Normalize(request);
            await ValidateAsync(normalized, rowId, token);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (request.FinalPackage && blocked.Length > 0)
            throw Validation("rows", "Final eSRS packages can include only reviewed or explicitly accepted subcontracting report data rows.");
    ...
    public async Task<IReadOnlyList<SubcontractingReportDataRowDto>> ImportCsvAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var requests = ParseCsv(csvContent);
        return await transaction.ExecuteAsync(async token =>
        {
    ...
    {
        string[] columns = ["contractId", "subcontractorId", "reportType", "reportPeriodStart", "reportPeriodEnd",
            "rowPeriodStart", "rowPeriodEnd", "socioeconomicCategory", "planCategory", "amount",
            "supportingEvidenceItemIds", "sourceReference"];
        return new("subcontracting-report-data-template.csv", columns, string.Join(',', columns) + Environment.NewLine);
    }

    private async Task ValidateAsync(SubcontractingReportDataRowRequest request, Guid? existingRowId, CancellationToken token)
    {
    ...
    private static IReadOnlyList<SubcontractingReportDataRowRequest> ParseCsv(string csvContent)
    {
        ...
        var expected = GetImportTemplate().Columns;
        ...
        for (var index = 1; index < records.Count; index++)
        {
            ...
            requests.Add(new(contractId, subcontractorId, reportType, reportStart, reportEnd, rowStart, rowEnd,
                cells[7], cells[8], amount, evidence, cells[11]));
        }
        return requests;
    }
...
{
    public bool IsPackageEligible => ReviewStatus is SubcontractingReportDataReviewStatus.Reviewed or SubcontractingReportDataReviewStatus.Accepted;
}
