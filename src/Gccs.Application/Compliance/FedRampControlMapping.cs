    ...
    IApplicationTransaction transaction)
{
    ...
    public async Task<FedRampControlMappingDto> CreateAsync(CreateFedRampControlMappingRequest request, Guid actorUserId, CancellationToken cancellationToken = default)
    {
        ...
        {
            var created = await repository.CreateAsync(tenantContext.TenantId, request, actorUserId, transactionToken);
            await WriteAuditAsync(created, actorUserId, AuditAction.Created, "FedRAMP readiness control mapping was created.", transactionToken);
            return created;
        }, cancellationToken);
    }
    ...
    public async Task<FedRampControlMappingDto?> LinkEvidenceAsync(Guid mappingId, FedRampEvidenceLinkRequest request, Guid actorUserId, CancellationToken cancellationToken = default)
    {
        ...
        return await transaction.ExecuteAsync(async transactionToken =>
        {
            var updated = await repository.LinkEvidenceAsync(tenantContext.TenantId, mappingId, request, actorUserId, transactionToken);
            if (updated is not null)
            {
                await WriteAuditAsync(updated, actorUserId, AuditAction.Updated, "FedRAMP control evidence link was added.", transactionToken);
            }
    ...
    public async Task<FedRampControlMappingDto?> AddGapAsync(Guid mappingId, FedRampGapRequest request, Guid actorUserId, CancellationToken cancellationToken = default)
    {
        ...
        return await transaction.ExecuteAsync(async transactionToken =>
        {
            var updated = await repository.AddGapAsync(tenantContext.TenantId, mappingId, request, actorUserId, transactionToken);
            if (updated is not null)
            {
                await WriteAuditAsync(updated, actorUserId, AuditAction.Updated, "FedRAMP control gap was recorded.", transactionToken);
            }
    ...
    public async Task<FedRampControlMappingDto?> ChangeStateAsync(Guid mappingId, FedRampControlReviewRequest request, Guid actorUserId, CancellationToken cancellationToken = default)
    {
        ...
        return await transaction.ExecuteAsync(async transactionToken =>
        {
            var current = await repository.GetAsync(tenantContext.TenantId, mappingId, transactionToken);
            if (current is null)
            {
                return null;
            }
            ...
            var updated = await repository.ChangeStateAsync(tenantContext.TenantId, mappingId, request, actorUserId, transactionToken);
            if (updated is not null)
            {
                await WriteAuditAsync(updated, actorUserId, AuditAction.Updated, $"FedRAMP control mapping moved to {updated.ReviewState}.", transactionToken);
            }
