...
public sealed class EfSecurityIncidentReadinessRepository(GccsDbContext db, ICurrentTenantContext context) : ISecurityIncidentReadinessRepository
{
    ...
    public async Task<IncidentReadinessRecordDto> SaveIncidentAsync(Guid tenantId, Guid actor, SaveIncidentReadinessRequest r, CancellationToken ct)
    {
        ...
        e.Tabletops = tabletopRows.Select(x => new IncidentTabletopRecordEntity { Id = x.RowId, LogicalId = x.LogicalId, TenantId = tenantId, ExecutedAt = x.Request.ExecutedAt, Environment = x.Request.Environment.Trim(), ParticipantsJson = JsonSerializer.Serialize(x.Request.Participants), FindingsJson = JsonSerializer.Serialize(x.Request.Findings), EvidenceReference = x.Request.EvidenceReference.Trim(), ReviewerUserId = x.Request.ReviewerUserId, EvidenceSourceType = x.Request.EvidenceSourceType, EvidenceFileVersionId = x.Request.EvidenceFileVersionId, ExternalUri = x.Request.ExternalUri?.Trim(), Sha256Digest = x.Request.Sha256Digest?.ToLowerInvariant() }).ToList();
    ...
    public async Task ValidateEvidenceSourcesAsync(Guid tenantId, IReadOnlyList<ReadinessEvidenceSource> sources, CancellationToken ct)
    {
        Scope(tenantId);
        var ids = sources.Where(x => x.SourceType == "EvidenceFileVersion" && x.EvidenceFileVersionId.HasValue).Select(x => x.EvidenceFileVersionId!.Value).Distinct().ToArray();
        if (ids.Length == 0) return;
        var valid = await db.EvidenceFileVersions.AsNoTracking().Where(x => ids.Contains(x.Id) && x.EvidenceItem!.TenantId == tenantId && x.DeletedAt == null && !x.IsUseBlocked && !x.EvidenceItem.IsUseBlocked &&
            x.ValidationStatus == "accepted" && x.MalwareScanStatus == "clean" && x.EvidenceItem.Status == Gccs.Domain.Evidence.EvidenceStatus.Approved).Select(x => x.Id).ToArrayAsync(ct);
        if (valid.Length != ids.Length) throw new CuiReadyApprovalChecklistValidationException("One or more evidence sources are unavailable, unsafe, unapproved, or outside the current tenant.");
    }
    public async Task<IReadOnlyList<ReadinessEvidenceOptionDto>> ListEvidenceOptionsAsync(Guid tenantId, CancellationToken ct)
    {
        Scope(tenantId);
        return await db.EvidenceFileVersions.AsNoTracking()
            .Where(x => x.EvidenceItem!.TenantId == tenantId && x.DeletedAt == null && !x.IsUseBlocked && !x.EvidenceItem.IsUseBlocked &&
                x.ValidationStatus == "accepted" && x.MalwareScanStatus == "clean" && x.EvidenceItem.Status == Gccs.Domain.Evidence.EvidenceStatus.Approved && x.FileHash != null)
            .OrderBy(x => x.EvidenceItem!.Name).ThenByDescending(x => x.VersionNumber)
            .Select(x => new ReadinessEvidenceOptionDto(x.Id, x.EvidenceItemId, x.VersionNumber, x.EvidenceItem!.Name, x.FileName, x.FileHash!, x.UploadedAt))
            .ToArrayAsync(ct);
    }
