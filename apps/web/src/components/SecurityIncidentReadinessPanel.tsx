...
export function SecurityIncidentReadinessPanel({ userId }: { userId: string | null }) {
  ...
  return <section className="members-section workflow-control-surface" aria-label="Security and incident readiness">
    ...
    {loading ? <p role="status">Loading readiness records…</p> : <div className="readiness-record-grid">
      <article className="evidence-list__item security-review-card"><div className="section-heading--split"><h3>Security review</h3><span className={`status status--${(security?.state ?? "draft").toLowerCase()}`}>{security?.state ?? "Not created"}</span></div>
        ...
        {findings.length > 0 ? <h4>Findings</h4> : null}{findings.map((finding,index)=><div className="evidence-list__item readiness-child-editor" key={finding.id}><label>Area<select value={finding.area} onChange={e=>setFindings(v=>v.map((x,n)=>n===index?{...x,area:e.target.value}:x))}>{areas.map(x=><option key={x}>{x.replaceAll("-"," ")}</option>)}</select></label><label>Summary<input value={finding.summary} onChange={e=>setFindings(v=>v.map((x,n)=>n===index?{...x,summary:e.target.value}:x))}/></label><label>Severity<select value={finding.severity} onChange={e=>setFindings(v=>v.map((x,n)=>n===index?{...x,severity:e.target.value}:x))}>{["Low","Medium","High","Critical"].map(x=><option key={x}>{x}</option>)}</select></label><label>Status<select value={finding.status} onChange={e=>setFindings(v=>v.map((x,n)=>n===index?{...x,status:e.target.value}:x))}>{["Open","Closed","AcceptedRisk"].map(x=><option key={x}>{x}</option>)}</select></label><label>Remediation owner<input value={finding.remediationOwner} onChange={e=>setFindings(v=>v.map((x,n)=>n===index?{...x,remediationOwner:e.target.value}:x))}/></label><label>Due date<input type="date" value={finding.dueAt??""} onChange={e=>setFindings(v=>v.map((x,n)=>n===index?{...x,dueAt:e.target.value||null}:x))}/></label><label>Closure notes<textarea value={finding.closureNotes??""} onChange={e=>setFindings(v=>v.map((x,n)=>n===index?{...x,closureNotes:e.target.value||null}:x))}/></label></div>)}
        {risks.length > 0 ? <h4>Accepted risks</h4> : null}{risks.map((risk,index)=><div className="evidence-list__item readiness-child-editor" key={risk.id}><label>Related finding<select value={risk.findingId??""} onChange={e=>setRisks(v=>v.map((x,n)=>n===index?{...x,findingId:e.target.value||null}:x))}><option value="">General review risk</option>{findings.map(f=><option key={f.id} value={f.id}>{f.summary||f.area}</option>)}</select></label><label>Scope<textarea value={risk.scope} onChange={e=>setRisks(v=>v.map((x,n)=>n===index?{...x,scope:e.target.value}:x))}/></label><label>Review date<input type="date" value={risk.reviewAt??""} onChange={e=>setRisks(v=>v.map((x,n)=>n===index?{...x,reviewAt:e.target.value||null}:x))}/></label><label>Expiration date<input type="date" value={risk.expiresAt??""} onChange={e=>setRisks(v=>v.map((x,n)=>n===index?{...x,expiresAt:e.target.value||null}:x))}/></label><label>Mitigation<textarea value={risk.mitigationNote} onChange={e=>setRisks(v=>v.map((x,n)=>n===index?{...x,mitigationNote:e.target.value}:x))}/></label></div>)}
        <div className="security-review-card__footer-actions">
          ...
          <button className="security-review-card__save" type="button" disabled={busy || !userId} onClick={()=>void save()}>Save new review version</button>
        </div>
