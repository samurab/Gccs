...
public sealed class ProductionReadinessChecklistTests
{
    ...
    public void TC_PR_6_2_Launch_candidate_tag_preconditions_are_complete()
    {
        ...
        Assert.Contains("Exact-candidate staging deployment", tagRecord);
        Assert.Contains("Pull-request CI", tagRecord);
        Assert.Contains("Hosted interaction review", tagRecord);
        ...
        Assert.Contains($"Created as `{manifest.ApprovedLaunchCandidateTag}`", checklist);
        Assert.Contains("docs/release/approved-launch-candidate.json", checklist);
    ...
    public void TC_PR_7_1_Production_deployment_uses_approved_ci_cd_and_launch_candidate()
    {
        ...
        var deploymentPending = deployment.Contains(
            "Deployment status: current approved candidate is awaiting protected production CI/CD execution",
            StringComparison.Ordinal);
        var deploymentCompleted = deployment.Contains(
            "Deployment status: current approved candidate deployed successfully through the protected production CI/CD path",
            StringComparison.Ordinal);

        Assert.True(
            deploymentPending ^ deploymentCompleted,
            "Production evidence must describe the current candidate as exactly one of pending or deployed.");
        ...
        if (deploymentPending)
        {
            ...
            Assert.Contains("Production secrets source | Historical path passed; current execution pending", deployment);
        }
        else
        {
            Assert.Matches(
                $"Current candidate execution status: `{Regex.Escape(manifest.ApprovedLaunchCandidateTag)}` deployed successfully in production workflow run `\\d+`\\.",
                deployment);
            ...
            Assert.Matches(
                $"Candidate `{Regex.Escape(manifest.ApprovedLaunchCandidateTag)}` deployed successfully in protected production workflow run `\\d+`",
                checklist);
        }

        Assert.Contains("Production environment configuration | Passed", deployment);
        Assert.Contains("PR71-PROD-DEPLOY-001", deployment);
        Assert.Contains("Evidence artifact `8843490287`", deployment);
    ...
    public void TC_PR_8_1_Daily_pilot_monitoring_covers_required_production_and_support_signals()
    {
        ...
        using var dailyReviewJson = JsonDocument.Parse(ReadText("output", "playwright", "production-readiness", "pr-8.1", "daily-monitoring-2026-07-22.json"));
        ...
        Assert.Contains("daily-monitoring-2026-07-14.json", monitoring);
        Assert.Contains("daily-monitoring-2026-07-14.json", checklist);
        Assert.Contains("daily-monitoring-2026-07-18.json", monitoring);
        Assert.Contains("daily-monitoring-2026-07-18.json", checklist);
        Assert.Contains("daily-monitoring-2026-07-19.json", monitoring);
        Assert.Contains("daily-monitoring-2026-07-19.json", checklist);
        Assert.Contains("daily-monitoring-2026-07-20.json", monitoring);
        Assert.Contains("daily-monitoring-2026-07-20.json", checklist);
        Assert.Contains("daily-monitoring-2026-07-21.json", monitoring);
        Assert.Contains("daily-monitoring-2026-07-21.json", checklist);
        Assert.Contains("daily-monitoring-2026-07-22.json", monitoring);
        Assert.Contains("daily-monitoring-2026-07-22.json", checklist);
    ...
    private static ApprovedLaunchCandidateManifest ReadApprovedLaunchCandidateManifest()
    {
        using var document = JsonDocument.Parse(ReadText("docs", "release", "approved-launch-candidate.json"));
        var root = document.RootElement;

        return new ApprovedLaunchCandidateManifest(
            root.GetProperty("approvedLaunchCandidateTag").GetString() ?? string.Empty,
            root.GetProperty("approvedCommitSha").GetString() ?? string.Empty,
            root.GetProperty("approvedDate").GetString() ?? string.Empty,
            root.GetProperty("dataPosture").GetString() ?? string.Empty);
    }

    private sealed record ApprovedLaunchCandidateManifest(
        string ApprovedLaunchCandidateTag,
