...
| Incidents and defects | `docs/production-readiness-pilot-monitoring.md` and `docs/production-readiness-launch-gap-decisions.md` | No new incident or defect record found; monitoring findings `PR81-MONITOR-001` and `PR81-MONITOR-002` are closed by external control evidence. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| PR81-MONITOR-001 | Alert owner receipt is proven by approved action-group receiver evidence and Azure Monitor delivery receipt. | Medium | Engineering lead | Keep receiver evidence current and reopen if the receiver is removed, ownership changes, or delivery fails. | Before production customer launch or alert-notification claims | Closed; controlled pilot may continue and alert-notification routing may be claimed only for the verified action group path. | Close `PR72-ALERT-ROUTE-001` and `PR81-MONITOR-001` in the known-risk log. |
| PR81-MONITOR-002 | Restore readiness is proven for the tested staging point-in-time restore path. | High | Engineering lead | Repeat restore rehearsal if backup settings, region, database version, migration baseline, networking, retention policy, or launch approval date materially changes. | Before production customer launch or recoverability claims | Closed for the tested staging restore path; do not claim geo-DR or production customer-data restore from this evidence. | Close `PR41-RESTORE-001` and `PR81-MONITOR-002` in the known-risk log. |
