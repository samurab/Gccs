...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PR43-MALWARE-001 | PR-4.3 | Production malware scanner is not enabled for upload workflows. | Launch blocker until scanner evidence or approved exception exists | Security owner | No-CUI posture, prohibited upload rejection, per-file No-CUI attestation, server-side validation, `scan-pending` default, blocked content download until scan is clean, audit logging, support escalation, and ability to disable production file upload paths. | Before production customer launch, or 30 days after exception approval, whichever comes first | Product owner and security owner | Approval pending | Production launch and launch candidate tagging remain blocked. |
