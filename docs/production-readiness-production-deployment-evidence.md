...
| --- | --- | --- |
| Approved launch candidate artifact | Passed | Tag `gccs-no-cui-mvp-lc-2026-07-08-2` points to `6c8927ec9cf79de977d76cb2594b87dd48f973bd`; see `docs/production-readiness-launch-candidate-tag.md`. |
...
| --- | --- | --- |
| TC-PR-7.1.1 | Passed | Production workflow checks `launch_candidate_tag` equals `gccs-no-cui-mvp-lc-2026-07-08-2` and checks out that tag. |
...
2026-07-04 dispatch attempt: `gh workflow run .github/workflows/production.yml --repo samurab/Gccs --ref codex/production-readiness-pr-7-2-smoke-tests -f launch_candidate_tag=gccs-no-cui-mvp-lc-2026-07-08-2` failed before deployment with `HTTP 404` because GitHub requires the workflow to exist on the default branch. Remote repository inspection also showed no GitHub `production` environment and no production variables/secrets available through the API.
