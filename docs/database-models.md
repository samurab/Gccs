...
- SSP export hardening migration: `src/Gccs.Infrastructure/Persistence/Migrations/20260910161637_HardenSspExportReviewPackages.cs`
...
| SSP builder | `ssp_sections`, `ssp_section_links`, `ssp_section_source_references`, `ssp_section_status_history`, `ssp_narratives`, `ssp_narrative_sources`, `ssp_export_packages`, `ssp_export_package_history`, `ssp_export_policies` | Tenant-scoped structured sections and narrative versions; server-resolved source snapshots and fingerprints; immutable human- and machine-readable review packages; versioned export-language policy snapshots; package-specific external-share approval; tenant-configurable independent-approver enforcement; classification and reviewer metadata; optimistic versions; one current approved narrative per section; and append-only audit integration. |
