...
| SSP builder | `ssp_sections`, `ssp_section_links`, `ssp_section_source_references`, `ssp_section_status_history`, `ssp_narratives`, `ssp_narrative_sources` | Tenant-scoped structured sections and narrative versions, server-resolved source snapshots and fingerprints, classification and reviewer metadata, optimistic versions, one current approved narrative per section, and append-only audit integration. |
