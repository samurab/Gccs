...
| Guarded assistant | `assistant_answers`, `assistant_draft_actions`, `assistant_feedback`; assistant-linked `expert_review_items` | Tenant-scoped sanitized answer outcomes, citation snapshots, explicit support/draft/review state, reviewable draft actions, user feedback, assignment/due-date workflow, and reviewer/timestamp/decision/notes traceability. Assistant escalations use source type `assistant_answer`; a filtered unique index permits one open escalation per tenant and answer while retaining resolved history. A reviewed answer remains draft guidance and these records do not authorize CUI/classified storage or autonomous workflow mutation. |
