...
| Reporting and audit | `reports`, `report_contracts`, `report_obligations`, `report_evidence`, `esrs_report_data_rows`, `esrs_report_data_evidence`, `audit_log_entries` | Generated reports, tenant-scoped eSRS preparation inputs with evidence and review metadata, report source scope, immutable activity/audit history, and tenant/type-scoped SPRS generation idempotency keys. |
...
- Subcontracting report data is preparation-only. Rows require a matching source-backed eSRS applicability period, a contract-linked subcontractor, and current-tenant evidence references. Edits reset review eligibility; only `Reviewed` or explicitly `Accepted` rows can enter final package preparation.
