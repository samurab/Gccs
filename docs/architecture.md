...
- SSP packages are immutable relational snapshots with human-readable content, JSON metadata, section/narrative/source/reviewer snapshots, evidence and POA&M references, the applied export-language policy version, version uniqueness per tenant, and append-only lifecycle history. Package persistence and export audit append share the relational transaction. A normalized Unicode-aware language policy rejects certification, compliance, assessor-determination, authorization, and government-endorsement claims before snapshot creation.
- `ExportReports` authorizes package generation and history reads. External sharing is a separate record-only lifecycle: `ManageTenant` records explicit package-specific approval, and the external-share-record endpoint fails closed until that approval exists. Tenant policy requires an approver other than the package generator by default; changing that versioned policy requires `ManageTenant` and writes an audit event atomically. Approval and external-share record transitions are audit logged. FeDril does not transmit the package; `/share` is retained only as a deprecated compatibility alias.
