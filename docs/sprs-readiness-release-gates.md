...
| Repeated or concurrent retries create one report for the same tenant, operation key, and request. | Implemented | `Idempotency-Key` is required, a canonical fingerprint prevents key reuse for different input, and a tenant/type-scoped PostgreSQL advisory lock plus unique index serializes concurrent requests. The web client retains a pending key across ambiguous failures. |
| Explicit CUI and export-control markings in notes declared Unclassified or FCI are rejected without storing the note. | Partially implemented | Server-side marker detection routes the attempt through the audited tenant data-handling policy. Heuristics cannot identify every unmarked or mislabeled sensitive passage. |
...
Execution status and required evidence are tracked in `docs/sprs-readiness-staging-evidence.md`. A checklist or local test is not a substitute for the authenticated deployed-environment run.
...
| Concurrency and retries | A network retry or simultaneous request could otherwise create duplicate immutable artifacts and audits. | Required tenant-scoped idempotency key, canonical request fingerprint, PostgreSQL transaction advisory lock, unique index, replay response, and conflict audit. | Product/engineering | Implemented; rerun relational concurrency coverage after transaction or persistence changes. |
| Unmarked or novel sensitive text | Marker detection cannot determine CUI status from arbitrary prose and can produce false negatives or conservative false positives. | Block explicit restricted markings, retain user confirmation, minimize free text, train users, and use the suspected-CUI incident path. Never describe the detector as comprehensive content classification. | Security/customer owner | Residual human/process risk; review detector patterns after incidents and policy changes. |
