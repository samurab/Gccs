...
| Production smoke tests | PR-7.2 | Blocked by `PR72-PROD-SMOKE-002`. Production deployment, health, login, tenant access, RBAC denial, No-CUI acknowledgement, upload guardrails, report generation, and audit visibility passed with synthetic-only data on 2026-07-05, but byte-level evidence upload fails closed because no production malware scanner endpoint is configured. | `docs/production-readiness-production-smoke-evidence.md`, `output/playwright/production-readiness/pr-7.2/authenticated-production-smoke.json`, `docs/production-readiness-pilot-onboarding.md` | Yes for PR-7.3 pilot onboarding and PR-8 post-launch control |
