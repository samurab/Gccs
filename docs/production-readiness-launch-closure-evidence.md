...
| Staging upload guardrails and report controls | PR-3.4 | Blocked. Staging health passed and local backend/API coverage exists, but authenticated live staging upload/report smoke checks cannot run without a signed-in staging session or approved smoke credential. | `docs/production-readiness-staging-upload-report-evidence.md`, `output/playwright/production-readiness/pr-3.4/staging-health.json`, `output/playwright/production-readiness/pr-3.4/authentication-blocker.json` | Yes |
