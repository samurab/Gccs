...
| Staging tenant isolation and RBAC | PR-3.3 | Automated backend/API tests passed. Live authenticated staging direct API checks are blocked because no scoped staging API smoke credential is available in the local environment. | `docs/production-readiness-staging-security-evidence.md` | Yes |
