# Production Readiness Story Test Prompts

Use these prompts to drive automated, smoke, manual, or evidence-based verification for each story in [production-readiness-story-test-cases.md](production-readiness-story-test-cases.md). Execute one story prompt at a time.

## Shared Test Prompt Requirements

Use these requirements for every production readiness test prompt:

- Treat the MVP as **No-CUI / compliance management only with synthetic CUI-ready demonstration workflows**.
- Use synthetic or non-sensitive data only. Do not introduce real customer CUI, production secrets, production logs, or customer-identifying data into test artifacts.
- Read `docs/production-readiness-story-test-cases.md`, `docs/production-readiness-phase-use-cases.md`, `docs/production-readiness-plan.md`, and the referenced story before testing.
- Capture setup data, exact steps, expected result, actual result, evidence location, defects, and missing coverage.
- For code-backed controls, prefer automated backend/API/frontend tests. For launch governance controls, use document review, grep-based checks, launch package review, approval evidence review, or smoke-test evidence.
- Any failed, skipped, manual-only, or missing verification must be classified as blocker, accepted risk, deferred follow-up, or not applicable with owner, mitigation, and target date.
- Preserve tenant isolation, server-side RBAC, audit logging, tenant mode enforcement, upload controls, report claim controls, source traceability, and review metadata.
- Run the narrowest relevant verification command and report results, or state why the command could not be run.

Recommended prefix:

```text
Using the GCCS production readiness artifacts and existing test patterns, execute or create verification for the following production readiness test cases. Use synthetic or non-sensitive data only. Capture setup, steps, expected result, actual result, evidence location, defects, missing coverage, and blocker/risk disposition. Run the narrowest relevant verification command and report results.
```
